...
def process_document(document_id: str):
    """
    * Step 1 : Download from S3 and extract text, tables, and images from the document.
    ...
    *   - `processing_details` : What type of elements or metadata did we retrieve from the document to show in the UI.
    ...
    try:
        ...
        # Step 1 : Download from S3 and extract content.
...
def download_content_and_partition(document_id: str, document: dict):
    """
    Download a file from S3, partition it into elements like text, tables, and images,
    then analyze the elements summary and upload it to db.
    """
    try:
...
def vectorize_chunks_summary_and_store_in_database(processed_chunks, document_id):
    ...
    try:
        ...
        for start in range(0, len(ai_summary_list), batch_size):
            ...
            while True:
                try:
                    embeddings = llm["embeddings"].embed_documents(batch_texts)
