...
def get_effective_settings(project_id):
    """Settings-Anker. Bei projektlosem Chat (ADR-004 / A1) fallen Defaults.

    Die Dokumenten-Zugriffsmenge wird unabhaengig hiervon ueber
    `accessible_document_ids(user_id)` (ADR-001) bestimmt -- cross-Projekt.
    """
    if not project_id:
...
def prepare_prompt_and_invoke_llm(
    user_query: str, texts: List[str], images: List[str], tables: List[str], chat_llm=None
) -> str:
    """
    ...
    response = (chat_llm or llm["chat_llm"]).invoke(messages)
...
def generate_query_variations(original_query: str, num_queries: int = 3) -> List[str]:
    ...
    try:
        ...
        structured_llm = llm["chat_llm"].with_structured_output(QueryVariations)
