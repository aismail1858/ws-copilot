...
def retrieve_context(project_id, user_query, user_id: Optional[str] = None):
    set_project_id(project_id)
    try:
        """
        RAG Retrieval Pipeline Steps:
        * Step 1: Get user's project settings from the database.
        * Step 2: Retrieve the document IDs the user may access (ADR-001 Zugriffsmenge).
        ...
        * Step 7: Build the context from the retrieved chunks and format them into a structured context with citations.
        """
        # Step 1: Get effective settings (project-scoped oder Defaults bei projektlosem Chat, ADR-004).
        project_settings = get_effective_settings(project_id)
        ...
        # Step 2: Zugriffsmenge gemaess ADR-001 (global/team/members/eigene).
        if user_id:
            document_ids = get_accessible_document_ids(user_id)
        else:
            document_ids = get_project_document_ids(project_id)
        logger.info("documents_found", document_count=len(document_ids), access_scoped=bool(user_id))
...
def vector_search(user_query, document_ids, project_settings):
    user_query_embedding = llm["embeddings"].embed_documents([user_query])[0]
