...
class ProjectCreate(BaseModel):
    ...
    team_id: Optional[str] = Field(None, description="Optionales Team, zu dem das Projekt gehört (admin/team_lead)")


class ProjectUpdate(BaseModel):
    name: Optional[str] = Field(None, description="The new name of the project")
    description: Optional[str] = Field(None, description="The new description of the project")


class ChatCreate(BaseModel):
    title: str = Field(..., description="The title of the chat")
    project_id: Optional[str] = Field(None, description="Optional project anchor (settings). Cross-project search runs over the user's accessible docs (ADR-001).")
...
class FileUploadRequest(BaseModel):
    ...
    visibility: Optional[str] = Field(None, description="global | team | private (admin darf 'global'; sonst aus Projekt-Team abgeleitet)")
