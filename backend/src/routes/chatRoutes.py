...
from src.models.index import ChatCreate, MessageCreate
from src.config.logging import get_logger, set_project_id, set_user_id
from src.services.chat_streaming import stream_chat_response
...
async def create_chat(
    chat: ChatCreate, current_user_id: str = Depends(get_current_user_id)
):
    """
    ! Logic Flow
...
async def delete_chat(
    chat_id: str, current_user_id: str = Depends(get_current_user_id)
):
    """
    ! Logic Flow
    * 1. Get current user user_id
...
async def stream_chat_message(
    chat_id: str, body: ChatStreamRequest, request: Request
):
    """
    Projektloser, user-scoped Chat-Stream (ADR-004 / A1).
    Sucht cross-Projekt ueber alle zugaenglichen Dokumente des Users
    (`accessible_document_ids`, ADR-001). `project_id` ist nur Settings-Anker.
    """
    ...
    project_id = chat_result.data[0].get("project_id")
    return stream_chat_response(project_id, chat_id, user_id, body.content, body.model_id)
