...
from src.models.index import ProjectCreate, ProjectSettings, ProjectUpdate
...
@router.get("/")
async def get_projects(current_user_id: str = Depends(get_current_user_id)):
    """
    ! Logic Flow
...
async def create_project(
    ...
    * 6. Return newly created project data
    ...
    if tier not in ("admin", "team_lead"):
        raise HTTPException(status_code=403, detail="Nur Admins/Team-Leads duerfen Projekte anlegen")
    team_id = project_data.team_id
    if team_id and tier != "admin":
        lead_check = (
            supabase.table("teams")
            .select("id")
            .eq("id", team_id)
            .eq("lead_id", current_user_id)
            .execute()
        )
        if not lead_check.data:
            raise HTTPException(status_code=403, detail="Du darfst keine Projekte fuer dieses Team anlegen")
    try:
...
async def update_project(
    ...
    set_user_id(current_user_id)
    try:
        logger.info("updating_project", project_id=project_id)
        ...
        if not update_result.data:
            raise HTTPException(status_code=422, detail="Failed to update project")

        logger.info("project_updated_successfully", project_id=project_id)
        return {
        ...
        logger.error("project_update_error", error=str(e), exc_info=True)
        raise HTTPException(
            status_code=500,
            detail=f"An internal server error occurred while updating project: {str(e)}",
        )
...
async def send_message(
    ...
    try:
        ...
        try:
            project_settings = await get_project_settings(project_id, current_user_id)
