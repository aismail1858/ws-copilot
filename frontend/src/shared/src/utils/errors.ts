 ...
 */
export function normalizeChatErrorDetail(raw: string): string {
 ...
 */
export function mapChatError(status: number | undefined, rawDetail: string, requestId?: string): string {
 ...
 */
export function mapIngestionError(status: number, detail: string): string {
  const normalized = detail.toLowerCase();
  ...
  if (
    ...
    return (
      'Upload aktuell nicht möglich: Der Storage-Dienst (MinIO) ist gerade nicht erreichbar ' +
...
export async function readHttpErrorDetail(res: Response): Promise<string> {
  ...
  if (!text) return 'Unbekannter Fehler';
  try {
